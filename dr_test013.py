...
# # Pandas 스타일링을 사용하여 데이터프레임의 모든 텍스트를 볼드체로 설정하고 크기를 증가
# styled_df = selected_data[['place', 'company', 'runner_name']].style.set_properties(
#     **{'font-weight': 'bold', 'font-size': '16pt'}
# )

# # 스타일링된 데이터프레임을 Streamlit에 표시
...
# st.markdown(f'✔ **Average Speed:** {average_speed} m/s')    
# st.markdown(f'✔ **Stride:** {average_stride} m')
# st.markdown(f'✔ **Cadence:** {average_cadence} steps/m')   

# st.markdown(f'<h3 style="color:black;">✔ <b>Average Speed:</b> {average_speed} m/s</h3>', unsafe_allow_html=True)
# st.markdown(f'<h3 style="color:black;">✔ <b>Stride:</b> {average_stride} m</h3>', unsafe_allow_html=True)
# st.markdown(f'<h3 style="color:black;">✔ <b>Cadence:</b> {average_cadence} steps/m</h3>', unsafe_allow_html=True)
