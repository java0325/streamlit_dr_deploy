# csv 파일에서 user와 passwd를 로딩하여 처리
...
#         df = pd.read_csv(file_path, encoding='cp949')  # cp949 인코딩 사용
...
# def check_login():
#     user_data = load_user_data()  # 사용자 데이터 로드
#     print("table user, password : ", user_data['user'], user_data['password'])
...
#         # 입력된 사용자 이름과 비밀번호가 CSV 파일의 데이터와 일치하는지 확인
#         if any((user_data['user'] == user) & (user_data['password'] == password)):
#             print("test!!!")
#             # print("table : user, password ", user_data['user'], user_data['password'])
#             st.session_state.logged_in = True
#             st.session_state.login_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
#         print("입력 user name:", st.session_state.get('username'))
#         print("입력 password:", st.session_state.get('password'))
#         user = st.session_state.username
#         password = st.session_state.password

#         # 입력된 사용자 이름과 비밀번호가 CSV 파일의 데이터와 일치하는지 확인
#         user_row = user_data[user_data['user'] == user]
#         print("로드된 csv user password:", user_row.iloc[0]['password'])
#         print("조건1 true?: ", not user_row.empty )
#         print("조건1.5 true?: ", user_row.iloc[0]['password'], password) 
#         print("조건2 true?: ", user_row.iloc[0]['password'] == password)       
#         # print("조건2 true?: ", not user_row.empty and user_row.iloc[0]['password'] == password)
...
# # 로그인 확인 함수
# def check_login():
#     # 사용자 이름과 비밀번호가 모두 입력된 경우에만 로그인 절차 진행
#     if st.session_state.get('username') and st.session_state.get('password'):
#         if st.session_state.username == expected_username and st.session_state.password == expected_password:
#             st.session_state.logged_in = True
#             st.session_state.login_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#         else:
#             st.error("잘못된 사용자 이름 또는 비밀번호입니다.")        
...
#     with st.sidebar:
#         st.title("로그인")
#         st.session_state.username = st.text_input("사용자 이름", on_change=check_login)
#         st.session_state.password = st.text_input("비밀번호", type="password", on_change=check_login)
