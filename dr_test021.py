# csv 파일에서 test020까지는 user와 password만 적용되었지만, 해당 데이터도 user1은 user1의 것이 user2는 user2의 것이 조회되도록 수정
# video encoding to H264
# ./data/kor20230728f00001_20230902_001/output_kor20230728f00001_20230902_001.mp4
# ./data/kor20230728f00001_20230902_001/output_kor20230728f00001_h264_20230902_001.mp4
# ffmpeg -i ./data/kor20230728f00001_20230902_001/output_kor20230728f00001_20230902_001.mp4 -vf scale=1280:720 ./data/kor20230728f00001_20230902_001/output_kor20230728f00001_h264_20230902_001.mp4
# ffmpeg -i ./data/kor20230902m00001_20230902_001/output_kor20230902m00001_20230902_001.mp4 -vf scale=1280:720 ./data/kor20230902m00001_20230902_001/output_kor20230902m00001_h264_20230902_001.mp4
...
def load_user_data():
    file_path = 'dr_rec004.csv'  # 사용자 데이터가 포함된 CSV 파일 경로
    try:
        df = pd.read_csv(file_path, encoding='utf-8', dtype=str)  # 모든 필드를 문자열로 처리
    except UnicodeDecodeError:
        df = pd.read_csv(file_path, encoding='cp949', dtype=str)  # cp949 인코딩 사용

    # 데이터 정리: 공백 및 특수 문자 제거
...
# 로그인 확인 함수
def check_login():
    user_data = load_user_data()  # 사용자 데이터 로드
    ...
    if st.session_state.get('username') and st.session_state.get('password'):
        ...
        # 입력된 사용자 이름과 비밀번호가 CSV 파일의 데이터와 일치하는지 확인
        user_row = user_data[user_data['user'] == user]
...
if 'logged_in' not in st.session_state or not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        st.info(f"로그인 시간: {st.session_state.login_time}")
        # 로그아웃 버튼에 로그아웃 함수 연결
        st.button("로그아웃", on_click=perform_logout)

    # 사용자에 따른 데이터 필터링
    file_path = 'dr_rec004.csv'
    df = pd.read_csv(file_path, encoding='cp949')  # 또는 encoding='euc-kr'

    # 현재 로그인한 사용자 이름에 따라 데이터 필터링
    user_specific_data = df[df['user'] == st.session_state.username]

    st.title('Video analysis results')

    # Sidebar에 날짜 선택 옵션 추가
    available_dates = user_specific_data['date'].unique()  # 현재 사용자의 데이터에서 날짜 목록 가져오기
    selected_date = st.sidebar.selectbox('Please choose a training date👏', available_dates)

    # 선택한 날짜에 해당하는 데이터 추출
    selected_data = user_specific_data[user_specific_data['date'] == selected_date].copy()

    # 데이터프레임을 HTML로 변환
    html_data = selected_data[['place', 'company', 'runner_name']].to_html(index=False)

    # CSS 스타일 정의 및 HTML 데이터 출력
    css_style = """
    <style>
        table {
            width: 100%;
...
#     st.markdown(html_data, unsafe_allow_html=True)
    ...
    # 선택한 날짜에 해당하는 데이터에서 비디오 파일 경로 추출
    filename = selected_data['video_file'].values[0]
